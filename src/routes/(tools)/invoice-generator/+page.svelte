<script lang="ts">
	...
	async function convertToPDF(pdf) {
		...
		if (selectedImage) {
			...
			const imgData =
				'"https://static.thenounproject.com/png/625182-200.png"';
			...
			<div id="invoice" class="w-full p-4 bg-white shadow sm:p-8 md:p-8 rounded-lg">
				...
				>
							...
							<div class="invoice-title ">
								<h2>
									<input type="text" bind:value={invoice} placeholder="Invoice" required />
								</h2>
							</div>
							<div class="logo">
								<div class="chan" on:click={()=>{Logo_fileinput.click();}}>
								</div>
								<input style="display:none" type="file" accept=".jpg, .jpeg, .png" on:change={handleFileChange} bind:this={Logo_fileinput} >
								{#if selectedImage}
								<img class="logo_image" src="{logo_image}" alt="" on:click={()=>{Logo_fileinput.click();}} />
								{:else}
								<img class="logo_image" src="https://static.thenounproject.com/png/625182-200.png" alt="" on:click={()=>{Logo_fileinput.click();}} />
								<span>Add Logo</span>
								{/if}
